...
namespace Cottle.Commons {
    public static class CommonFunctions {
        ...
        //public static readonly IFunction FunctionWhere = new CallbackFunction(delegate(IList<Value> values, Scope scope, TextWriter output) {
        //    IFunction callback = values[1].AsFunction;

        //    return values[0].Fields.Where(x => callback.Execute(new Value[] { x.Value }, scope, output).AsBoolean).ToList();
        //}, 2);
        #endregion
